...
registry.decl(BlockNodeName, MagicNode, {
    ...
    alterArch: function() {
        ...
        return function() {
            ...
            if (arch.hasNode(grouping)) {
                ...
                blockNode = new Node.Node(grouping);

                arch.setNode(
                    blockNode,
                    arch.getParents(this)
                        .filter(this.linkParent, this),
                    this);
            }

            return this.createChildNodes(blockNode);

        };

    },

    createChildNodes: function(parent) {
        var _this = this;
        ...
        return Q.when(this.level.getItemsByIntrospection(),
            function(items) {
                ...
                items.forEach(function(item) {
                    ...
                    var node = _this.createTechNode(item, parent, _this);

                    if (node && ~optTechs.indexOf(item.tech))
                        _this.createOptimizerNode(item.tech, node, parent);
    ...
    linkParent: function(parentId) {
        var parent = this.ctx.arch.getNode(parentId);
        return !(parent instanceof TargetNode.TargetNode) || ('=' + this.getId()) === parentId;
    },
    ...
    createDefaultTechNode: function(item, parentNode, expandingNode) {
        ...
        var arch = this.ctx.arch,
            node = new EntityNode.EntityNode({
                root: this.root,
                level: this.level,
                item: item
            });
    ...
    createBorschikOptimizerNode: function(tech, sourceNode, parentNode) {
        ...
        var files = sourceNode.getRelPaths();
    ...
    createId: function(o) {
        ...
        var id = this.__base(o),
            item = o.item || o;

        if (!item.suffix) return id;

        return id.slice(0, -(item.suffix.length));
    }
