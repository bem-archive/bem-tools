    ...
    buildByDecl: function(decl, levels, output, opts) {
        ...
        return Q.all(levels
            ...
            .then(function(){
                ...
                if (typeof opts.declaration === 'string') declLastModified =
                    QFS.lastModified(opts.declaration)
                        .fail(function() {
                            return -1;
                        });
...
var Deps = exports.Deps = INHERIT(/** @lends Deps.prototype */{
    ...
    walk: function (items, predicate, action){
        var ids = items.map(function(item){
            return item.buildKey();
        });
        this._walk(ids, this.items[''], predicate, action);
    },

    _walk: function(keys, ctx, predicate, action){
        var _this = this;
        keys.forEach(function(key){
            if(_this.items[key]){
                var item = _this.items[key];
                if(predicate(item, ctx)){
                    _this._walk(item.mustDeps, item, predicate, action);
                    action.call(_this, item, ctx);
                    _this._walk(item.shouldDeps, item, predicate, action);
                }
            }
        });
    },
