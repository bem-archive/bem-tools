...
registry.decl(BemCreateNodeName, GeneratedFileNode, {
    ...
    make: function() {
        ...
        worker.on('exit', function(code) {
            LOGGER.fdebug("Exit of bemcreate worker for node '%s' with code %s", _this.getId(), code);
            handler({ code: code });
        });

        worker.on('message', function(m) {
            LOGGER.fdebug("Message from bemcreate worker for node '%s': %j", _this.getId(), m);
            handler(m);
        });
        /* jshint +W109 */
    ...
    /**
     * Get files minimum or maximum mtime in milliseconds. -1 or Infinity in case of any file doesn't exist.
     * @param {Boolean} [max] Pass true to return maximum mtime.
     * @return {Promise * Number}
     */
    lastModified: function(max) {
        var _this = this;
        
        return Q.all(this.getFiles()
            .map(function(path) {

                return QFS.lastModified(PATH.join(_this.root, path))
                    .fail(function() {
                        return max? Infinity: -1;
                    });

            }))
            .spread(max? Math.max: Math.min);
