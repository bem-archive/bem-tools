...
Указания в БЭМ-дереве `{ block: button }` достаточно, чтобы сборщик прошёл по всем уровням переопределения проекта и
...
Для того, чтобы в сборку клиентского JS попали шаблоны блока `b1` с модификатором `_m1_v1`, зависимости в `deps.js`-сущности
