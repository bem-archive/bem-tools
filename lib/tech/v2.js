var Q = require('q'),
    ...
    Tech = exports.Tech = INHERIT(/** @lends Tech.prototype */ {
        ...
        getCreateResults: function(prefix, vars) {
            ...
            return Q
                .all(this.getCreateSuffixes().map(function(suffix) {
                    return _this.getCreateResult(_this.getPath(prefix, suffix), suffix, vars)
                        .then(function(r) {
                            res[suffix] = r;
                        });
                }))
                ...
                return Q.all(_this.getBuildSuffixes()
                    .map(function(destSuffix) {
                        var filteredFiles = files[destSuffix] || [],
                            file = _this.getPath(PATH.resolve(outputDir, outputName), destSuffix);
                        ...
                        return _this.validate(file, filteredFiles, opts)
                            .then(function(valid) {
                                ...
                                if (!valid) {
                                    ...
                                    return _this.getBuildResult(filteredFiles, destSuffix, outputDir, outputName, opts)
                                        .then(function(r) {
                                            res[destSuffix] = r;
                                        });
                                }
        ...
        validate: function(file, files, opts) {
            ...
            if (!opts.force) {
                Q.all([_this.getLastUsedData(file),
                       QFS.exists(file)])

                    .spread(function(prevFiles, exists) {
                        prevFiles = prevFiles.buildFiles;
                        if (prevFiles && prevFiles.length === files.length)
                            valid.resolve(_this.sameFiles(files, prevFiles) && exists);
                        else
                            valid.resolve(false);
                    });
            }
            else valid.resolve(false);
        ...
        saveLastUsedData: function(file, data) {
            ...
            return bemUtil.writeFile(path, JSON.stringify(data));
        },
