...
exports.Context = INHERIT({
    ...
    createTech: function(name, path) {
        if (this.getLevel()) return createTech(this.getLevel().resolveTechClass(name), name, this.getLevel()).setContext(this);
        return TECH.createTech(this.resolveTech(path || name), name);
    },

    resolveTech: function(techIdent) {
        return (typeof techIdent !== 'string' && techIdent) || (bemUtil.isPath(techIdent) && this.resolveTechPath(techIdent)) ||
