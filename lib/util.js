...
exports.levelOptParse = function() {
    ...
    return this.opt()
        ...
        .short('l')
        .long('level')
        .apply(exports.applyLevelOpt())
        .end();

};

exports.applyLevelOpt = function(levelDesc) {
    ...
    return function() {
        ...
        return this
            .def(def)
            .title([levelDesc + ' directory path, default: ',
...
/**
 * Executes specified command using child_process.fork()
 * @param cmd {String} cmd Command to execute.
 * @param args {Array} [args] Array of arguments to pass to child_process.fork()
 * @param msg {Object} [msg] Object to pass to the child process after fork being done. Executing command can handle it
 * using process.on('message', handler) call.
 * @returns {Promise * Object | Undefined} Object which is passed to the parent process by child using process.send().
 * Child should quit on receiving 'term' message.
 */
exports.fork = function(cmd, args, msg) {

    var worker = CP.fork(cmd, args),
        d = Q.defer(),
        handler = function(m) {
            (m.code !== 0)? d.reject(m.msg) : d.resolve(m.data);
        };
...
var isTechModule = exports.isTechModule = function (path) {
    try {
