...
var Level = exports.Level = INHERIT(/** @lends Level.prototype */{
    ...
    getFileByObjIfExists: function(item, tech) {
        //if (Array.isArray(item.elem)) console.log('filebyobj %j %j %j', item, this.getRelByObj(item), this.getPaths(this.getRelByObj(item), tech));
//        var paths = this.getPaths(this.getRelByObj(item), tech),
//            res = [];
    ...
    'match-block': function(path) {
        ...
        console.log('match-block: %s', ['^(' + this.matchRe() + ')',
                    '\\1(.*?)$'].join(PATH.dirSepRe));
    ...
    'match-block-mod': function(path) {
        ...
        console.log('match-block-mod: %s', ['^(' + m + ')',
                    '_(' + m + ')',
                    '\\1_\\2(.*?)$'].join(PATH.dirSepRe));
    ...
    'match-block-mod-val': function(path) {
        ...
        console.log('match-block-mod-val: %s', ['^(' + m + ')',
                    '_(' + m + ')',
                    '\\1_\\2_(' + m + ')(.*?)$'].join(PATH.dirSepRe));
    ...
    'match-elem': function(path) {
        ...
        console.log('match-elem: %s', ['^(' + m + ')',
                    '__(' + m + ')',
                    '\\1__\\2(.*?)$'].join(PATH.dirSepRe));
    ...
    'match-elem-mod': function(path) {
        ...
        console.log('match-elem-mod: %s', ['^(' + m + ')',
                    '__(' + m + ')',
                    '_(' + m + ')',
                    '\\1__\\2_\\3(.*?)$'].join(PATH.dirSepRe));
    ...
    _scanFiles: function() {
        ...
        return bemUtil.fsWalkTreeCb(_this.dir, function(fileData) {
            ...
            console.dir(o);
//            if (!o) console.log('not matched %s', fileData.relPath)
        })
            .then(function() {
                console.log('level %s scanned %s files', _this.dir, Object.keys(list).length);

                _this.files = files;
//                console.log('blocks %j', blocks);
                bemUtil.mkdirs(_this.dir);
                console.log('writing %s', _this.dir);
