...
var Deps2 = exports.Deps2 = INHERIT({
    ...
    expandByFS: function() {
        ...
        // TODO: do expandOnceByFS() until the dependence does not no longer increase

        var _this = this,
            depsCount1 = this.getCount(),
            depsCount2,
            i = 10;
        return _this.expandOnceByFS().then(function again() {
        ...
        return newDeps.clone(this);

    },

    expandOnceByFS: function() {
        var newDeps = this.clone();
        return Q.step(this.map(function(item) {
                return function() {
    ...
    expandItemByFS: function(item) {
        ...
        return Q.step(tech.getContext().getLevels().map(function(level) {
    ...
    forEach: function(fn, uniq, itemsByOrder) {
        ...
        (itemsByOrder || this.itemsByOrder).forEach(function(i) {
            i = _this.items[i];
            var key = i.buildKey();
