...
var Level = exports.Level = INHERIT(/** @lends Level.prototype */{
    ...
    __constructor: function(path) {
        ...
        this.relDir = PATH.relative(projectRoot, this.dir);
        ...
        for(var e in exceptLevels) {
            ...
            if (this.relDir.substr(0, except.length) === except) {
    ...
    scanFiles: function(force) {
        ...
        var _this = this,
            cachePath = PATH.join(projectRoot, '.bem', 'cache', this.relDir, 'files.json');

        if (this.cache) {
            try {
                _this.files = JSON.parse(FS.readFileSync(cachePath));
                return;
        ...
        bemUtil
            ...
            .then(function() {
                bemUtil.writeFile(cachePath, JSON.stringify(files));
            })
