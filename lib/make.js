...
exports.createArch = function(root) {
    ...
    return QFS.exists(rootMakefile)
        ...
        .then(function() {
            var regArch = Make.getRegistry('arch'),
                Arch = regArch || require('./default-arch').Arch;

            LOGGER.silly("Initing DefaultArch");

            return new Arch(arch, root).alterArch();
        });
...
var Make = exports.NodesRegistry = {
    ...
    arch: function(object) {
        var base = Make.getRegistry('arch') || require('./default-arch').Arch;
        registry['arch'] = INHERIT(base, object);
    },
