 ...
 */
    createLevel = exports.createLevel = function(level, opts) {
...
var Level = exports.Level = INHERIT(/** @lends Level.prototype */{
     ...
     */
    getPath: function(prefix, tech, suffix) {
        return this.getTech(tech).getPath(prefix, suffix);
    },
    ...
    getRelPathByObj: function(item, tech) {
        return this.getPath(this.getRelByObj(item), tech, item.suffix);
    },
     ...
     * @param {String[]} [suffixes]  Array of the suffixes to return paths for.
     * If not specified tech.getSuffixes() will be used.
     ...
     */
    resolveBemPath: function(bemPath, suffixes) {
        ...
        if (part && part.tech) {
            ...
            if (bemPath.length > 1) {
                if (bemUtil.isLevel(path)) {
                    return createLevel(path).resolveBemPath(bemPath.slice(1), suffixes);
                }
            } else {
                return this.getTech(part.tech).getPaths(this.getByObj(part), suffixes);
            }
        }
     ...
     */
    resolveBemRelPath: function(bemPath, suffixes) {

        return this.resolveBemPath(bemPath, suffixes)
            .map(function(path) {
    ...
    _ignorePathRe: /(\.svn|\.git|\.bem|node_modules)$/,
