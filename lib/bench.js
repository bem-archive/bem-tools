    ...
    return Q.all(treeish.map(function(trish) {
        return gitIterator( trish ) 
            .then(function(path_to_treeish) {
                ...
                bemjson_files.forEach(function(file) {

                    var bemjson = fs.readFileSync( file , 'UTF-8');
                    var bemhtml = require(file.replace(/.bemjson.js/,'.bemhtml.js'));
                    var context = vm.createContext({});
                    var res = vm.runInContext(bemjson, context, file);
                    
                    
                    var suite = new Benchmark.SuiteSSS;
                    // add tests
