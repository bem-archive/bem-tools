        ...
        .act(function(opts) {
            ...
            return QFS.exists(rootMakefile)
                .then(function(exists) {
                    ...
                    /* jshint +W109 */
                    if (exists) return (new SERVER(opts)).start();

                    try {
                        LOGGER.finfo('Try to use ENB');

                        var enb = require('enb/node_modules/commander');
                        require('enb/lib/cli/server.js')(enb);
                        enb.parse(process.argv);
                    } catch(err) {}
