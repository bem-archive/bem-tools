...
### Сборка технологий CSS, BEMHTML и локализационной технологии JS, из файлов декларации (bemdecl) для уровня страниц `pages`. Подключаются уровень переопределения блоков `blocks`, а также `blocks-common` и `blocks-desktop` из `bem-bl`
...
### Сборка `merged`-бандла (раньше так же назывался `common`)
