var Q = require('qq'),
    ...
    Tech = exports.Tech = INHERIT(/** @lends Tech.prototype */ {
         ...
         * @returns {Promise * Undefined}
         */
        build: function(decl, levels, outputDir, outputName, opts) {
            return this.storeBuildResults(
                PATH.resolve(outputDir, outputName),
                this.getBuildResults(decl, levels, outputDir, outputName, opts));
        },
         ...
         */
        getBuildResult: function(files, suffix, outputDir, outputName, opts) {
            var _this = this;
            ...
            return files.then(function(files) {
                _this.getBuildSuffixes()
                    .forEach(function(destSuffix) {
                        ...
                        if (!opts.force) {
                            var prevFiles = _this.getLastUsedFiles(PATH.resolve(outputDir, outputName), destSuffix);
                            if (prevFiles && prevFiles.length === filteredFiles.length) {
                                valid = _this.sameFiles(filteredFiles, prevFiles) &&
                                    PATH.existsSync(_this.getPath(PATH.resolve(outputDir, outputName), destSuffix));
                            }
                        }

                        if (!valid) {
                            _this.saveLastUsedFiles(PATH.resolve(outputDir, outputName), destSuffix, filteredFiles);

                            res[destSuffix] = _this.getBuildResult(filteredFiles, destSuffix, outputDir, outputName, opts);
                        }

                        console.log('file %s is %s', PATH.resolve(outputDir, outputName) + '.' + destSuffix, valid?'valid':'not valid');
                    });
        ...
        sameFiles: function(now, old) {
            for(var i = 0; i < now.length; i++) {
                ...
                if (n.absPath !== o.absPath ||
                    ...
                    console.log('%j not equal %j %s %s', n, o, n.absPath !== o.absPath, n.lastUpdated != o.lastUpdated);
                    console.log('%s %s', (n.lastUpdated+0), (o.lastUpdated+0));
                    return false;
                }
            }
        ...
        getLastUsedFiles: function(prefix, suffix) {
            var path = this.getPath(prefix, suffix) + '.meta.js',
                files = null;

            try {
                files = JSON.parse(FS.readFileSync(path)).buildFiles;
            } catch(err) {
        ...
        saveLastUsedFiles: function(prefix, suffix, files) {
            var path = this.getPath(prefix, suffix) + '.meta.js';

            return bemUtil.writeFile(path, JSON.stringify({buildFiles: files}));
        },
                            ...
                            decl.deps && decl.deps.forEach(function(dep) {
                                ...
                                if (files) {
                                    for(var i = 0; i < files.length; i++) {
                                        var file = files[i],
                                            buildSuffixes = suffixesMap[file.suffix];

                                        if (buildSuffixes) {
                                            buildSuffixes.forEach(function(buildSuffix) {
