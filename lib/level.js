...
var Level = exports.Level = INHERIT(/** @lends Level.prototype */{
        ...
        if (this.cache) {
            ...
            return this.filesPromise = bemUtil.readFile(PATH.join(_this.dir, 'files.json'))
                .then(function(cacheContent) {
                    ...
                    console.log('level %s read from cache %s files', _this.dir, Object.keys(_this.files).length);
                })
                .fail(function(err) {
    ...
    _scanFiles: function() {
        ...
        return bemUtil.fsWalkTreeCb(_this.dir)
            .then(function(files) {
                console.log('level %s scanned %s files', _this.dir, Object.keys(files).length);
                _this.files = files;
