...
Отметим, что функция `require.resolve()` проверяет наличие файла модуля технологии по указанному пути. Если путь был задан в виде `'foo': 'путь/до/модуля/технологии/foo.js'`, то при отсутствии файла по указанному пути поведение технологии будет идентично базовой.
...
В пакете bem-tools с версии 0.6.4 было обновлено API базовой технологии, используемое при написании модулей технологий. В дальнейшем будем называть обновленное API базовой технологии v2, а старое v1. 

Изложенное в предыдущих разделах верно как для модулей, реализованных с поддержкой API v1, так и для модулей API v2.

**Важно**: Было изменено только API базовой технологии команды `bem build` и команд, использующих `bem build` в ходе выполнения (например `bem make`).

С изменениями в API v2 базовой технологии можно ознакомиться в разделах:

 * [Изменения сигнатур методов в базовом классе технологии API v2](#Изменения-сигнатур-методов-в-базовом-классе-технологии-api-v2)
 * [Стандартный ход выполнения методов базовой технологии с API v2](#Стандартный-ход-выполнения-методов-базовой-технологии-с-api-v2)
...
Для начала работы с обновленным API рекомендуется воспользоваться версией [project-stub](https://github.com/bem/project-stub/tree/master), использующей API v2. На данный момент поддержка API v2 добавлена в ветку `master`.
...
Если в файле level.js проекта или уровня переопределения стандартные модули технологий задекларированы не были, то по умолчанию будет использоваться API v1. Для использования новой API v2 путь к стандартным модулям технологий v2 должен быть задан явно, как показано в примере выше.
...
Если сборка запущена с опцией `--force`, содержимое кеша перезаписывается.
...
При повторных сборках проекта часто возникает ситуация, когда собираемые файлы уже присутствуют в результирующем каталоге сборки. В таких ситуациях требуется определить, актуален ли файл, имеющийся на диске, или его требуется пересобрать. Этот процесс называется **валидацией** файла.

В API v1 проверка валидности собираемого файла выполнялась с помощью процедуры `bem make`, точнее - кодом в BemBuildNode. 
...
| ------------- |-------------|------------- |-------------|------------- |-------------|
| -d | Путь к файлу декларации сборки | Зависимости блоков, процес сборки | Объект | Да | decl |
...
| -t  | Название технологии или полный путь к модулю технологии | Суффиксы собираемых и создаваемых файлов, выбор модулей технологии. | Строка  | Нет | - |
...
На третьем этапе из фрагментов данных для каждого суффикса формируется объект `res`, содержащий массив результатов сборки. Объект `res` возвращается методом `getBuildResults` в виде промиса. В ключах этого объекта лежат суффиксы создаваемых файлов, а в значениях — содержимое этих файлов.
...
Для API v2 базовой технологии команды `bem build` точкой входа по-прежнему является метод `buildByDecl(decl, levels, output, opts)`. 
Промежуточный метод `build` был исключен из цепочки вызовов базовой технологии v2.

Метод `buildByDecl` вызывает `getBuildResults(decl, levels, output, opts)`, результат работы которого - это хеш. Ключом в хеше служит суффикс собираемых файлов технологии, а значением - массив строк содержимого соответствующих файлов. Например, для технологии `i18n.js` он может выглядеть так:
```js
{
 'en.js': ['...', '...', ...],
 'ru.js': ['...', '...', '...', ...],
...
Этот хеш передается методу `storeBuildResults(prefix, res)`, который сохраняет его содержимое в файлы бандла технологии с соответствующим суффиксом.
...
Каждый путь файла обрабатывается методом `getBuildResultChunk(relPath, path, suffix)`, который на выходе формирует строку содержимого для текущего файла. В зависимости от особенностей технологии это может быть как путь к собираемому файлу, обернутый в директиву подключения, так и содержимое этого файла.
