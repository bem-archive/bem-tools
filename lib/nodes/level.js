    ...
    alterArch: function(ctx) {
        ...
        return function() {
            ...
            decl.forEach(function(block) {
                ...
                if (!arch.hasNode(itemNodeClass.createId(_this.level, block.name))) {

                    arch.setNode(
                        new itemNodeClass(_this.level, block.name),
                        levelNode,
                        thisNode);
                }
    ...
    actualizeElements: function(ctx, newBlockDecl, oldBlockDecl, node) {
        ...
        if (newBlockDecl.elems) {
            newBlockDecl.elems.forEach(function(elem) {
                ...
                if (!arch.hasNode(itemNodeClass.createId(_this.level, newBlockDecl.name, elem.name))) {
                    arch.setNode(
