...
var Benchmark = INHERIT({
    ...
    defineBackups: function() {
        var allBackups = [];

        if (!this.withoutCurrent) {
            LOGGER.info('Include ' + 'current_state'.magenta);
            this.treeishList = [this.pathSelf].concat(this.treeishList);
        } else {
            LOGGER.info('Exclude ' + 'current_state'.magenta);
            if (!this.noMake) allBackups = this.backupRevision();
        }
        ...
        if (!self.noMake) {
            ...
            return Q.when(self.cleaner())
                .then(function() {
                    ...
                    return Q.all(self.defineBackups())
                        .then(function() {
                            if (!self.withoutCurrent) {
