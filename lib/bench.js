...
var Benchmark = INHERIT({
        ...
        if (self.benchmarks && self.benchmarks.length) {
            ...
            self.benchmarks
                .map(function(item) {
                    ...
                    if (self.isOnlyLevel(item)) {
                        paths = self.getBemjsonPathsByLevel(treeishPath, level);
                    } else {
                        paths = [PATH.join(
                            process.cwd(),
                            treeishPath,
                            item,
                            self.getBlock(item) + '.bemjson.js'
                        )];
                    }
            ...
            benchmarks = self.getBenchLevelsByPath(treeishPath)
                .map(function(item) {
                    return {
                        'level': item,
                        'bemjsonPaths': self.getBemjsonPathsByLevel(treeishPath, item)
                    };
                });
        }
    ...
    /**
     * Check on exist only level in path
     * 
     * @param  {String}  path path to block or only level
     * @return {Boolean}      true if path is level, else false
     */
    isOnlyLevel: function (path) {
        var slashIndex = path.indexOf('/');
        if (slashIndex === -1) return true;
        if (slashIndex === path.length -1) return true;
        return false;
    },
     ...
     */
    getBemjsonPathsByLevel: function(projectPath, level) {

        return LEVEL.createLevel(PATH.join(projectPath, level))
    ...
    makeTarget: function(target) {
        ...
        envClone.PATH = process.cwd() + '/node_modules/.bin' + ':' + process.env.PATH;
    ...
    getBenchLevelsByPath: function(path) {
        ...
        return LEVEL.createLevel(path)
            .getItemsByIntrospection()
                .filter(function(item) {
                    return item.tech === 'benchmarks' && true;
                })
                .map(function(item) {
                    return PATH.join(item.block + item.suffix);
                });
            ...
            .then(function() {
                ...
                return Q.all(targets
                    .map(function(target) {
                        ...
                        var links = self.getBenchmarksPaths(PATH.join(self.pathTmp, target));
