...
registry.decl(LibraryNodeName, Node, /** @lends LibraryNode.prototype */ {
    ...
    __constructor: function(o) {
        ...
        this.npmPackages = o.npmPackages || ['package.json'];
    },
    ...
    installLibraryDependencies: function() {
        ...
        if (this.npmPackages === false) {
            LOGGER.finfo('npmPacakges config variable is set to false, skip installing npm dependencies for the %s library', package, _this.target);
            return;
        }
...
registry.decl(GitLibraryNodeName, ScmLibraryNodeName, /** @lends GitLibraryNode.prototype */ {
    ...
    getUpdateCmd: function(url, target) {
        return UTIL.format('cd %s && git checkout HEAD~0 && git branch -D %s ; git fetch origin && git checkout --track -b %s origin/%s', target, this.treeish, this.treeish, this.treeish);
    }
