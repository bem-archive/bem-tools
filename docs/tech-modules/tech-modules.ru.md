...
На практике в качестве технологии могут рассматриваться самые разные данные, относящиеся к БЭМ-сущности. Документация блока тоже является одной из технологий, в которой блок реализован.
...
Модуль технологии работает с файлами технологии соответствующего ему типа. Как правило, для каждого типа данных создается отдельный модуль технологии, определяющий, каким образом обрабатывать данные этого типа. 
...
С одной стороны, для каждой БЭМ-сущности в проекте создаются отдельные файлы для каждой из технологий. Выше мы рассматривали эту особенность БЭМ-проектов на примере блока «Кнопка». Проект может включать сотни и даже тысячи БЭМ-сущностей, которые должны размещаться на файловой системе по определенной схеме.
...
Для получения списка файлов, попавших в предыдущую сборку, используется метод `getLastUsedData(file)`, для сохранения в кеш — `saveLastUsedData(file, data)`. Параметр `file` — это абсолютный путь к файлу, кеш которого нужно загрузить или сохранить, `data` — объект со списком файлов, который сохранится в кеш в виде JSON.
 ...
 * Метод `getBuildPaths()` по переданной декларации `decl` и списку уровней переопределения `levels` возвращает список фактически существующих на диске файлов, попадающих под декларацию. Список - это хеш, в котором файлы сгруппированы по суффиксу технологии. Например:
