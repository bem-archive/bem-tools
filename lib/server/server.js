        ...
        .then(function(exists) {
            if(exists) {
                ...
                var server = http.Server(
                    ...
                    function(e) {
                        console.log(util.format('Server initilize error: %s', e.message));
                    }).listen(opts.port);

            } else {
                console.log(util.format('%s does not exist', root));
            }
        });
}
...
function processFile(filePath, res) {
    ...
    } else if(filePath.match(/_.*\.(js|css)$/i) && !fs.existsSync(filePath)) {
