     ...
     var refresh = function() {
         $.getJSON('snapshots', function(data) {
             ...
             $.each(data, function(key, val) {
                 ...
                 items.push('<li class="b-snapshots_list_ul_li"><a class="b-snapshots_list_ul_li_a" href="snapshots/' + val + '">' +
                     val.replace(/^\d+\_/, '').replace(/\.json$/, '')
                     + '</a></li>');
             });

             $('<ul/>', {
                 class: 'b-snapshots_list_ul',
                 html: items.join('')
             }).appendTo('.b-snapshots_list');

             $('.b-snapshots_list_ul_li_a').click(function(item) {
                 $('#archpanel').empty();
                 processSnapshot(item.srcElement.href);
                 return false;
             });
         })
     };
