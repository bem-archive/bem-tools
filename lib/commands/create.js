...
var Q = require('q'),
    PATH = require('path'),
    UTIL = require('../util'),
    MakeReader = require('../make-reader.js'),
...
module.exports = function() {
    ...
    return this
        ...
        .cmd()
            ...
            .end()
        .apply(UTIL.chdirOptParse)
        .apply(UTIL.techsOptParse)
        .opt()
            .name('level').short('l').long('level')
            .def(UTIL.findLevel(process.cwd()))
        ...
        .act(function(opts, args) {
            UTIL.setEnv(opts);
            MakeReader.get().read(PATH.join(opts.root, '.bem', 'make.js'), ['levels']);
            ...
            var context = new Context(opts.level, opts),
                ...
                eachElem = function(itemBase, elem) {
                    var item = UTIL.extend({ elem: elem }, itemBase);
                ...
                eachMod = function(itemBase, mod) {
                    var item = UTIL.extend({ mod: mod }, itemBase);
                ...
                eachModVal = function(itemBase, val) {
                    items.push(UTIL.extend({ val: val }, itemBase));
                },
