...
var BemBuildMetaNode = exports.BemBuildMetaNode = INHERIT(BemBuildNode, {
    ...
    isValid: function() {
        ...
        var decl = wrappedRequire(PATH.resolve(this.root, this.declPath), true),
            ...
            start = new Date(),
            _this = this,
            prefixes = tech.getBuildPrefixes(tech.transformBuildDecl(decl, this.getId()), this.levels)
                .then(function(p) {
                    LOGGER.fdebug('getBuildPrefixes for %s took %s', _this.getId(), new Date()-start);
                    return p;
                }),
            start2 = new Date(),
            filteredPaths = tech.filterPrefixes(prefixes, tech.getSuffixes(), _this.getId())
                .then(function(p) {
                    LOGGER.fdebug('filteredPaths for %s took %s', _this.getId(), new Date()-start2);
                    return p;
                })
