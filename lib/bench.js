...
var Benchmark = INHERIT({
    ...
    runBenchmark: function(links, target) {
        ...
        return Q.all(links.map(function(link) {
            ...
            var def = Q.defer(),
                ...
                bh = require(link.replace(/.bemjson.js/, '.bh.js')),
            ...
            if (typeof bemhtml.BEMHTML !== 'function') {
                isBemhtml = false;
            }
            if (typeof bh.INST !== 'function') {
                isBh = false;
            }
            ...
            (function() {
                ...
                while (i--) {
                    isBemhtml ? bemhtml.BEMHTML.apply(res) : 0;
                    isBh ? bh.INST.apply(res) : 0;
                }
            })();
            ...
            if (!self.techs || self.techs.indexOf('bh') !== -1) {
                if (isBh) {
                    suite
                        .add(name + '(bh)'.underline, function() {
                            bh.INST.apply(res);
                        });
                }
            }
            if (!self.techs || self.techs.indexOf('bemhtml') !== -1) {
                if (isBemhtml) {
                    suite
                        .add(name + '(bemhtml)'.underline, function() {
    ...
    getResultsTable: function(results, targets) {
        ...
        for (var i = 1; i < sortedRes[0].length; i += 1) {
            ...
            if (!this.techs || this.techs.length > 1) {
                if (this.isBh && this.isBemhtml) {
                    if (i % 2 === 0 && i !== sortedRes[0].length-1) {
                        data.push([]);
                    }
                }
            }
