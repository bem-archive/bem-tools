...
registry.decl(EntityNodeName, Node, {
    ...
    getPaths: function() {
        return this.rootLevel.resolveBemPath(this.resolved, this.item.suffix);
    },

    getRelPaths: function() {
        return this.rootLevel.resolveBemRelPath(this.resolved, this.item.suffix);
    },
    ...
    isValid: function() {
        ...
        if (!children.length) {
            return parent.then(
                function(min) {
                    LOGGER.fdebug('*** isValid(%s): no deps => file existence %s min %s', _this.getId(), min > -1, min);
                    return min > -1;
                });
        }
