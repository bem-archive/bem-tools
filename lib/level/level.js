...
var Level = exports.Level = INHERIT(/** @lends Level.prototype */{
     ...
     * @return {Tech}
     */
    getTech: function(name) {
        if(!this._techsCache.hasOwnProperty(name)) {
            this._techsCache[name] = this.createTech(name);
        }
     ...
     * @return {Tech}
     */
    createTech: function(name) {
        return TECH.createTech(this.resolveTechClass(name), name, this);
    },
     ...
     * @return {Object}  Tech module path.
     */
    resolveTech: function(techIdent, force) {
        var path = techIdent;
        if(bemUtil.isPath(techIdent)) {
            path = this.resolveTechPath(techIdent);
        } else if(!force && this.getTechs().hasOwnProperty(techIdent) && this.getTechs()[techIdent]) {
            path = this.resolveTechPath(this.getTechs()[techIdent]);
        } else {
            path = bemUtil.getBemTechPath(techIdent);
        }
    ...
    /**
     * Resolve tech identifier into tech class.
     *
     * @param {String|Function|Object} techIdent  Tech identifier or function which return tech object or tech object.
     * @param {Object} [opts] Optional parameters
     * @return {Object}  Tech module class.
     */
    resolveTechClass: function(ident, opts) {
        ...
        if (this._techsClassCache.hasOwnProperty(ident)) return this._techsClassCache[ident];

        var path = ident;
        if(bemUtil.isPath(ident)) {
            path = this.resolveTechPath(ident);
        } else if(!opts.force && this.getTechs().hasOwnProperty(ident) && this.getTechs()[ident]) {
            var tech = this.getTechs()[ident];

            if (typeof tech === 'string') {
                path = this.resolveTechPath(this.getTechs()[ident]);
            } else if (typeof tech === 'object') {
               path = tech;
            } else if (typeof tech === 'function') {
                if (tech.prototype.__self) path = tech;

                path = tech(getBem());
            }
        } else {
            path = bemUtil.getBemTechPath(ident, opts);
        }

        return (this._techsClassCache[ident] = TECH.getTechClass(path, this));
    },
