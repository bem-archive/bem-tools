var fs = require('file'),
    os = require('os'),
    Tech = require('./bembuild/tech').Tech,
    Level = require('./bembuild/level').Level,
    parser = exports.parser = (new (require('args').Parser)())
        .option('-d', '--decl', 'declaration')
            ...
            .set()
            .validate(function (d) { return require(d) })
            .end()
        ...
        .option('-o', '--output-dir', 'outputDir')
            ...
            .def(fs.path(system.getcwd() + '/'))
            .set()
            .validate(function (d) { return fs.path(d.replace(/[^\/]$/, '$&/')) })
            .end()
...
exports.main = function () {
    ...
    options.tech.forEach(function (tech) {
            ...
            forItemWithMods = function (block, elem) {
                    ...
                    item.mods && item.mods.forEach(function (mod) {
                        ...
                        mod.vals.forEach(function (val) {
                            eachLevel(type + '-mod-val', args.concat(mod.name, val));
                        });
        ...
        tech.outPrefixes(prefixes, options.outputDir, options.outputName);
