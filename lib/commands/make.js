            ...
            return QFS.exists(rootMakefile)
                .then(function(exists) {
                    ...
                    if (!exists) {
                        ...
                        var targets = opts.method === 'clean' ? ['clean'] : args.targets,
                            indexOfAll = targets.indexOf('all');

                        indexOfAll > -1 && targets.splice(indexOfAll, 1);

                        try {
                            return ENB.make(targets, {
                                dir: opts.root,
                                mode: process.env.YENV === 'production' ? 'production' : 'development',
                                cache: !opts.force,
                                hideWarnings: opts.verbosity === 'error'
                            });
                        } catch(err) {}
                    ...
                    LOGGER.warn('Native bem make is deprecated. Use ENB config instead.');
