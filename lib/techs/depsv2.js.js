            ...
            .then(function(){
                ...
                Q.all([_this.getLastUsedData(file),
                    ...
                    .spread(function(data, lastUpdated) {
                        if (!data || !~lastUpdated) return valid.resolve(false)

                        valid.resolve(!_this.checkLevelsChanged(levels, data, lastUpdated));
                    })
...
var Deps = exports.Deps = INHERIT({
    ...
    expandItemByFS: function(item) {
        ...
        return Q.all(levels.map(function(level) {
            var files = level.getFileByObjIfExists(item.item, tech);
            if (files) {
                return Q.all(files.map(function(file) {
                    return tech.readContent(file.absPath)
                        .then(function(content) {
                            if (content) {
                                try {
                                    _this.parse(VM.runInThisContext(content, file.absPath), item);
                                } catch(e) {
                                    e.message = file.absPath + '\n' + e.message;
                                    throw e;
                                }
                            }
                        })
