...
var BundleNode = exports.BundleNode = INHERIT(BlockNode, {
    ...
    setBemBuildNode: function(techName, techPath, declPath, bundleNode, magicNode, forked) {
        ...
        buildNode.getFiles().forEach(function(f) {
            ...
            var alias = new FileNode({ path: f, root: this.root });
    ...
    createBorschikOptimizerNode: function(tech, sourceNode, bundleNode) {
        ...
        LOGGER.fdebug('Creating borschik nodes for %s', sourceNode.getFiles().join('\n'));

        return (sourceNode.getFiles? sourceNode.getFiles() : sourceNode.path)
            .map(function(file) {
