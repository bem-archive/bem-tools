...
registry.decl(BundleNodeName, BlockNode, /** @lends BundleNode.prototype */ {
    ...
    alterArch: function() {
        ...
        return function() {
            ...
            var arch = ctx.arch,
                bundleNode,
                grouping = PATH.dirname(this.rootLevel.resolveBemRelPath(this.resolved)[0]);

            console.log('grouping from %j %s', this.resolved, grouping)
...
//                });
    ...
    setBemBuildNode: function(techName, techPath, declPath, bundleNode, magicNode, forked) {
        ...
        console.log('meta node %s deps %j', metaNode.getId(), metaNode.getDependencies());
    ...
    setEntityNode: function(tech, bundleNode, magicNode) {
        var arch = this.ctx.arch,
            filePath = this.getBundlePath(tech);
        console.log('tech %s filepath %s', tech, filePath)
        if (!PATH.existsSync(PATH.resolve(this.root, filePath))) return;
        console.log('item %j extend %j', this.item, U.extend({}, this.item, {tech: tech, suffix: '.bemjson.js'}));
        ...
        console.log('created node %j', node);
        arch.setNode(node);
    ...
    'create-bemdecl.js-node': function(tech, bundleNode, magicNode) {
        ...
        var a = this.setBemCreateNode(
            tech,
            this.level.resolveTech(tech),
            bundleNode,
            magicNode);

        console.log('bemdecl created node %s', (a && a.getId()) || null);
        return a;
    },

    'create-deps.js-node': function(tech, bundleNode, magicNode) {

        var a = this.setBemBuildNode(
            tech,
            this.level.resolveTech(tech),
        ...
        console.log('depsjs created node %s', (a && a.getId()) || null);
