...
var Benchmark = INHERIT({
    ...
    __constructor : function(opts, args){
        ...
        this.withCurrent    = opts['without-current-state'];
        this.onlyBench      = opts['only-bench'];
        this.benchmarks     = opts.benchmarks;

        this.treeishListChecked = [];
        this.benchmarkNoFounded = [];

        this.WAITING           = opts['wait'] ? opts['wait'] : 20;
    ...
    getBemjsonPath : function(treeshPath, levelPath) {
        ...
        if (self.benchmarks) {
            self.benchmarks.forEach(function(benchmark) {
                ...
                if (FS.existsSync(path)) {
                    ...
                    if (self.benchmarkNoFounded.indexOf(benchmark) === -1) {
        ...
        if (bemEntities) {
             bemEntities.forEach(function(entity){
