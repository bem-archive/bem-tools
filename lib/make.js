...
exports.APW = INHERIT(APW, {
    ...
    findAndProcess: function(targets) {
        ...
        //XXX: Why Q.all? foundNodes is just an array of strings
        return Q.all(foundNodes)
            .then(function() {

                return _this.process(foundNodes)
                    .fin(function() {
                        _this.arch.removeNode(foundNodes[0]);
                    });
            });
    },
    ...
    createTargetNode: function(id) {
        ...
        //XXX: anyToUnifed бросит исключение если не сможет распарсить id, а не вернет false 
        ...
        if (id.match(/\/_[^\/]+$/)) {
            ...
            //XXX: use PATH.dirSep instead of '/'
            id = id.replace('/', ':');
            targetId = '=' + id;
        }
