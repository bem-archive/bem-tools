...
Context.prototype.resolveTech = function(techIdent) {
    ...
    if(this.levels.length) {
        techPath = this.resolveTechName(techIdent);
    }
    return (techPath || bemUtil.getBemTechPath(techIdent));
};

Context.prototype.resolveTechName = function(techIdent) {
    if (!this.levels.length) return null;
    var techPath = null;
    bemUtil.arrayReverse(this.levels).forEach(function(level) {
        !techPath && (techPath = level.resolveTechName(techIdent));
    });
    return techPath;
};
