var Q = require('qq'),
    ...
    Tech = exports.Tech = INHERIT(/** @lends Tech.prototype */ {
                ...
                _this.getBuildSuffixes()
                    .forEach(function(destSuffix) {
                        var srcSuffixes = _this.getBuildSuffixesMap()[destSuffix],
                            filteredFiles = _this.filterFilesBySuffixes(files, srcSuffixes);
        ...
        filterFilesBySuffixes: function(files, suffixes) {
            var filteredFiles = [];

            suffixes.forEach(function(suffix) {
                filteredFiles = filteredFiles.concat(files[suffix]||[]);
            });
                            ...
                            decl.deps && decl.deps.forEach(function(dep) {
                                ...
                                if (files) {
                                    for(var i = 0; i < files.length; i++) {
                                        var file = files[i],
                                            suffix = file.suffix;//_this.probeSuffix(file.relPath) || file.suffix;
