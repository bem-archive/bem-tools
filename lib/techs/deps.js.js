...
var Deps = exports.Deps = INHERIT({
    ...
    /**
     * Substracts specified Deps object from the current one
     * @param {Deps} deps - Unwished deps object
     * @returns {Deps}
     */
    ...
    /**
     * Filters current object items by specified one
     * @param {Deps} deps
     * @returns {Deps}
     */
    ...
    /**
     * Returns length of stored items
     * @returns {Number}
     */
    getCount: function() {
...
var DepsItem = exports.DepsItem = INHERIT({
    ...
    extendByCtx: function(ctx) {
        if(ctx && (ctx = ctx.item)) {
            var keys = ['block', 'elem', 'mod', 'val', 'tech'],
                key;
