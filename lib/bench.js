...
var Benchmark = INHERIT({
    ...
    exportGitRevision: function(treeish) {
        ...
        archive.on('close', function(code){
            if (code !== 0) {
                LOGGER.error('exportGitRevision - '.blue + '[' + treeish.green + '] ' + errChunk);
                def.reject(new Error(errChunk));
            } else {
    ...
    exportWorkingCopy: function() {
        ...
        rsync.on('close', function(code){
            if (code !== 0) {
                LOGGER.error('exportWorkingCopy - '.blue + 'Code:' + code + '\n' + err_chunk);
            } else {
                LOGGER.timeEnd(label);
     ...
     */
    getBemjsonPath: function(treeishPath, levelPath) {

        var self = this,
            level =  LEVEL.createLevel(treeishPath + levelPath),
            bemjsonFiles = [],
            benchmarks;
        ...
        benchmarks.forEach(function(benchmark) {
            ...
            else if (self.benchmarksNotFound.indexOf(benchmark) === -1) {
                self.benchmarksNotFound.push(benchmark);
            }
        });
    ...
    make: function(targets) {
        ...
        .then(function(links_pack) {
            ...
            if (self.benchmarksNotFound) {
                self.benchmarksNotFound.forEach(function(benchmark) {
                    LOGGER.warn('benchmark - ' + benchmark.red + ' not found!');
                });
            }
    ...
    /**
     * Copy reference benchmarks from specified path into every checked out revision
     *
        ...
        return Q.all(targets.map(function(target) {
            ...
            return U.exec(cmd)
                ...
                .then(function() {
                    ...
                    var cmd = 'cp -R ' + PATH.join(self.pathTmp,
                        (self.withoutCurrent ? self.latestRevision : self.pathSelf),
    ...
    getLatestTreeish: function() {
        ...
        .then(function(dates) {
            ...
            self.latestRevision = lastTreeish; /*save to public var*/
            LOGGER.info('Latest revision is - ' + lastTreeish.magenta);

            return lastTreeish; /*not used*/
            ...
            return Q.when(self.cleaner())
                .then(function() {
                    ...
                    if (!self.withoutCurrent) {
                        ...
                        allBackups = [self.exportWorkingCopy()].concat(self.backupRevision());
                    } else {
                        LOGGER.info(bcsExclude);
                        allBackups = self.backupRevision();
                    }
