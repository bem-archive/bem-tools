...
registry.decl(BemBuildNodeName, GeneratedFileNode, {
    ...
    isValid: function() {
        var _this = this;
        if (this.tech.getTechName() === 'deps.js') return true;
...
registry.decl(BemBuildMetaNodeName, BemBuildNodeName, {
    ...
    isValid: function() {
        ...
        var decl = U.readDecl(PATH.resolve(this.root, this.declPath)),
            tech = this.tech,
            techv2 = !!this.tech.getBuildFiles,
            relativize = getPathRelativizer(this.root),
            prefixes = !techv2? tech.getBuildPrefixes(tech.transformBuildDecl(decl), this.levels): Q.resolve([]),
            filteredPaths = !techv2? tech.filterPrefixes(prefixes, tech.getSuffixes()).invoke('map', relativize)
                : tech.getBuildFilesFlat(decl, this.levels).invoke('map', function(f) {
                    return relativize(f.absPath)
                }),
