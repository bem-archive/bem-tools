...
Подробнее о стандартных модулях можно узнать из [http://wiki.yandex-team.ru/users/ragnar/bemtools-tech-module/techmodule#prilozhenie1.perechenstandartnyxmodulejstexnologijs Приложения 1.]
...
```js transformBuildDecl: function(decl) {
...
**Пример:** В качестве примера рассмотрим модуль технологии [https://github.com/eprev/eprev.github.io/blob/5bc7b59a3f396b4ab05f39b480f46a71879fb32b/.bem/techs/xml.js xml.js]. Метод `getBuildResult(prefixes, suffix, outputDir, outputName)` переопределен в модуле таким образом, что отдельные фрагменты данных помещаются в конструкцию `<fest:template xmlns:fest="http://fest.mail.ru">` ... `</fest:template>`, задающую пространство имен XML.

```js    getBuildResult: function (prefixes, suffix, outputDir, outputName) {
...
**Описание:** Для команды `bem create` модуль работает идентично базовой технологии. Команда `bem build` собирает ссылки на js-файлы блоков в результирующий JS-файл, для дальнейшей обработки программой [http://github.com/bem/borschik borschik]. Ссылки имеют вид `/*borschik:include:путь/до/файла/имя_сущности.js*/;`.
...
```
    getCreateResults(prefix, vars) // Формирует содержимое файлов для всех создаваемых суффиксов
      ...
      getCreateResult(path, suffix, vars) § // Формирует содержимое файла для отдельного суффикса```
