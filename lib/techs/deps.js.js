...
exports.Tech = INHERIT(Tech, {
    ...
    transformBuildDecl: function(decl, id) {

        var deps = new Deps(decl.blocks || decl.deps);

        var start = new Date();

        return deps.expandByFS(this, id).then(function() {
            var expand = new Date();
            LOGGER.fdebug(' expand for %s took %s', id, (expand - start));
            ...
            LOGGER.fdebug('transformBuildDecl for %s took %s', id, (new Date() - start));
            return o;
        });
    ...
    getBuildResult: function(prefixes, suffix, outputDir, outputName) {
        ...
        var declaration = this.getContext().opts.declaration,
            deps = new Deps(declaration.deps || declaration.blocks);
...
var Deps = exports.Deps = INHERIT({
    ...
    parse: function(deps, ctx, fn, id) {
        ...
        //LOGGER.fdebug('parse for %s took %s', id, (new Date()) - start);
        return this;
    },

    expandByFS: function(tech, id) {
        ...
        var _this = this,
            ...
            depsCount2,
            i = 10;
        return _this.expandOnceByFS(id).then(function again(newDeps) {
            depsCount2 = newDeps.getCount();
            if(depsCount1 !== depsCount2 && i--) {
                depsCount1 = depsCount2;
                return newDeps.expandOnceByFS(id).then(again);
            }
    ...
    expandOnceByFS: function() {
        ...
        if (!steps.length) return newDeps;
    ...
    expandItemByFS: function(item, id) {
