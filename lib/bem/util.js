...
exports.merge_techs = function (level, options) {
    // FIXME: если при создании блока/элемента/модификатора
    // указали --force-tech <name>, и в level.js определена
    // технология с таким именем, нужно использовать именно её
    var techs = options.forceTech.length ? {} : level.techs;

    options.forceTech.concat(options.addTech).forEach(function(t) {
        var name = t.getTechName();
        !techs[name] && (techs[name] = t);
    });

    options.noTech.forEach(function(t) {
        delete techs[t.getTechName()];
    });
