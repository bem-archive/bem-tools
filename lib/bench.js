...
BemBenchmark.prototype.generateTable = function(results) {
    ...
    for (var i = 1; i < results[0].length; i += 1) {
        ...
        for (var j = 0; j < results.length; j++) {
            ...
            if(results[j][i]) {
                t.push('['+results[j][i].hz.green + '] ±' + results[j][i].rme.magenta + '%');
                if (results[j][0] !== this.LATEST_REVISION) {
                    sq += Number(results[j][i].hz);
                } else if (curr === 0) {
                    curr = Number(results[j][i].hz) + this.DELTA;
                }
                rme.push(Number(results[j][i].rme));
            } else {
                t.push('none');
            }
        }
        ...
        sq = Number(sq / (results[0].length - 1) ).toFixed(1);
        ...
        data.push( [i,name.replace('.bemjson.js','')].concat(t).concat([stat,perc])); // status ?
    }


    header.push('status'.magenta);
    header.push('rme'.magenta);
