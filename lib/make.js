...
function getIncludeSyncFunc(resolvePath) {

    return function(path) {
        return includeSync(resolvePath(path));
    }
...
function include(path) {
    ...
    return BEMUTIL.readFile(path)
        .then(function(content) {
            return evalConfig(content, path);
        });

}

function includeSync(path) {
...
function evalConfig(content, path) {
    ...
    VM.runInNewContext(
        ...
        BEMUTIL.extend({}, global, {
            ...
            includeSync: getIncludeSyncFunc(resolvePath)
        }),
        path);
