...
function evalConfig(content, path) {
    ...
    var resolvePath = getPathResolver(path),
        makeJsModule = new Module(path, module);

    // tune module instance to let require() work in that module
    makeJsModule.filename = path;
    makeJsModule.paths = Module._nodeModulePaths(PATH.dirname(path));

    var requireFunc = function(path) {
        return makeJsModule.require(path);
    };

    // let require.resolve() to work in make.js modules
    requireFunc.resolve = function(request) {
        return Module._resolveFilename(request, makeJsModule);
    };
