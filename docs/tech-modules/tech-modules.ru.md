...
Файлы технологии, как правило, идентифицируются модулем на основании суффиксов. **Суффиксом** в bem-tools считается часть имени файла, следующая после первой точки в его названии.
...
Таким образом, механизм для решения два типов задач, характерных для БЭМ-проектов, был реализован в архитектуре пакета bem-tools в виде команд `bem create` и `bem build`: 
...
В процессе создания модулей технологий из [всех команд](http://ru.bem.info/tools/bem/commands/), входящих в пакет bem-tools, ключевыми являются `bem create` и `bem build`. Команды `bem make` и `bem server` — это надстройки над ними. Это означает, что модули технологий, созданные для `bem create` и `bem build`, используются также командами `bem make` и `bem server`:
  ...
  * bem make на основании конфига сборки проекта make.js вызывает команды bem build и bem create с нужными аргументами, чтобы собрать бандлы всех нужных технологий проекта;
...
Базовая технология служит основой для модулей технологии. Модули переопределяют методы API технологии, реализованные в базовой технологии. Методы API, которые не были переопределены в модуле, будут работать в соответствие с базовой технологией.
...
Стандартные модули технологий удобно использовать как пример при создании собственных модулей. Любой из стандартных модулей может быть использован как основа для новой технологии.
...
Модули технологий, использующие API v1, будут работать с новой версией bem-tools без прироста скорости. Это относится как к готовым модулям технологий, идущим в составе `bem-tools`, так и к тем, которые входят в библиотеку [bem-bl](https://github.com/bem/bem-bl/tree/0.3).
