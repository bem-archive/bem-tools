...
registry.decl(LevelNodeName, MagicNode, {
    ...
    __constructor: function(o) {
        ...
        this.__base(o);
        this.thisLevel = createLevel(o.level.getPathByObj(o.item, o.item.tech));
    },
    ...
    alterArch: function() {
        ...
        return function() {
            ...
            if (arch.hasNode(grouping)) {
                levelNode = arch.getNode(grouping);
            } else {
                levelNode = new Node.Node(grouping);

                arch.setNode(
                    levelNode,
                    arch.getParents(this)
                        .filter(function(p) {
                            var node = ctx.arch.getNode(p);
                            return !(node instanceof TargetNode.TargetNode) || ('=' + this.getId()) === p;
                        }, this),
                    this);
            }
            ...
            // put the nodes which are not present in the decl anymore into an array for further removal
            ...
            decl.forEach(function(item) {
                ...
                if (!arch.hasNode(ItemNodeClass.createId(o))) {
                    arch.setNode(new ItemNodeClass(o), levelNode);
                }
    ...
    scanLevelItems: function() {
        return _.uniq(
            _.sortBy(
                this.thisLevel.getItemsByIntrospection()
    ...
    getLevelPath: function() {
        return PATH.relative(this.root, this.thisLevel.dir);
    },
...
registry.decl(BundlesLevelNodeName, LevelNodeName, {
    ...
    mergedBundleItem: function() {
        return { block: this.mergedBundleName(), tech: 'tech' };
    },
    ...
    scanLevelItems: function() {
        ...
        var sourceTechs = this.getBundleSourceTechs();
