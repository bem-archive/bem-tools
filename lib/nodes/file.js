...
registry.decl(FileNodeName, Node, /** @lends FileNode.prototype */ {
    ...
    make: function() {
        ...
        return QFS.exists(this.getPath())
            .then(function(exists) {
                if (!exists) return Q.reject(new Error(UTIL.format("Path %j doesn't exist. Node %s", _this.getPath(), _this.getId())));
            });
    ...
    lastModified: function() {
        ...
        return U.lastModified(this.getPath())
            .fail(function() {
                return -1;
            });
...
registry.decl(GeneratedFileNodeName, FileNodeName, /** @lends GeneratedFileNode.prototype */ {
    ...
    isValid: function() {
        ...
        if (this.ctx.method && this.ctx.method != 'make') return Q.fulfill(false);
        if (this.ctx.force) return Q.fulfill(false);
        ...
        if (!children.length) return Q.fulfill(false);
