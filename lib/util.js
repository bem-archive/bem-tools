...
exports.symbolicLink = function(link, target, force) {
    ...
    return Q.fulfill(force)
        .then(function(force) {

            // TODO: replace statLink with alternative
            return;
            if (!force) return;
        ...
        .then(function() {
            return QFS.symLink(target, link);
        });
...
exports.removePath = function(path) {
    ...
    return QFS.isDir(path)
        .then(function(isdir) {
            if (isdir) return QFS.removeDir(path);
...
exports.filterPaths = function(paths) {
    ...
    paths.forEach(function(path, index) {
        ...
        PATH.exists(path, function(exists) {
            ...
            d.fulfill(paths.filter(function(path, index) {
                return res[index];
            }));
...
exports.getDirsAsync = function(path) {
    return QFS.listDir(path).then(function(items) {
        return Q.all(items.map(function(i) {
            return QFS.isDir(PATH.join(path, i))
                .then(function(isDir){
                    return {
                        name: i,
                        dir: isDir
...
exports.getDirsFiles = function(path, dirs, files) {
    return QFS.listDir(path).then(function(list) {
        return Q.all(list
            .map(function(i) {
                return QFS.isDir(PATH.join(path, i))
                    .then(function(isDir) {
...
exports.exec = function(cmd, options, resolveWithOutput) {
    ...
    cp.on('exit', function(code) {
        if (code === 0) return d.fulfill(resolveWithOutput && output ? output : null);
...
/**
 * Returns modification time stamp for specified file.
