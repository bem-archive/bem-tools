...
var Q = require('q'),
    ...
    Tech = exports.Tech = INHERIT(/** @lends Tech.prototype */ {
        ...
        createByDecl: function(item, level, opts) {
            this.showDeprecateMessage(level);
            opts = opts || {};
        ...
        buildByDecl: function(decl, levels, output) {
            this.showDeprecateMessage(this.context.getLevel());
        ...
        showDeprecateMessage: function(level) {
            var techDesc = '"' + this.getTechName() + '" at ' + this.getTechPath();

            if (level) {
                techDesc += " configured at level " + level.dir;
            }

            bemUtil.deprecate('Tech module ' + techDesc + ' is using API V1.',
