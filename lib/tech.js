var Q = require('qq'),
    ...
    Tech = exports.Tech = INHERIT(/** @lends Tech.prototype */ {
         ...
         */
        filterPrefixes: function(prefixes, suffixes, id) {
            var _this = this;
            return Q.when(prefixes, function(prefixes) {
                ...
                prefixes.forEach(function(prefix) {
                    suffixes.forEach(function(suffix) {
                        ...
                        res[path] = PATH.existsSync(path); //QFS.exists(path);
                        //res[path] = QFS.exists(path);
                        paths.push(path);
                    });
                });

                LOGGER.fdebug('prefixes.forEach (%s) took %s', id, new Date()-start);

                start = new Date();
//                return Q.shallow(res).then(function(res) {
                    LOGGER.fdebug('shallow (%s) took %s', id, new Date()-start);
                    start = new Date();
                    var result = paths.filter(function(path) {
                        return res[path];
                    });
                    LOGGER.fdebug('paths.filter (%s) took %s', id, new Date()-start);
                    return result;
//                });
