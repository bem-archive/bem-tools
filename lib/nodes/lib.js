...
registry.decl(GitLibraryNodeName, ScmLibraryNodeName, /** @lends GitLibraryNode.prototype */ {
    ...
    getUpdateCmd: function(url, target) {
        ...
        return UTIL.format('git ' + gitDir + ' fetch %s %s git ' + gitDir + ' checkout %s %s', this.origin,
            onBranch? ('&& git ' + gitDir + ' branch ' + this.branch + '; '): ' && ',
            this.treeish || this.branch,
            onBranch? '&& git ' + gitDir + ' rebase ' + this.origin + '/' + this.branch: '');
    },
    ...
    getInfo: function(path) {
        ...
        var _this = this,
            gitDir = '--git-dir ' + PATH.join(path, '.git') + ' --work-tree ' + path,
            cmd = UTIL.format('git %s show-ref --head', gitDir),
            d = Q.defer();
