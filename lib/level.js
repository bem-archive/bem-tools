...
var Level = exports.Level = INHERIT(/** @lends Level.prototype */{
    ...
    scanFiles: function(force) {
        ...
        if (this.cache) {
            try {
                _this.files = JSON.parse(FS.readFileSync(PATH.join(_this.bemDir, 'cache', 'files.json')));
                return;
        ...
        var cachePath = PATH.join(_this.bemDir, 'cache');
        bemUtil
            .mkdirp(cachePath)
            .then(function() {
                bemUtil.writeFile(PATH.join(cachePath, 'files.json'), JSON.stringify(files));
            })
    ...
    scanBlock: function(path, block, items) {
        ...
        dirs.forEach(function(dir) {
            ...
            files.forEach(function(file) {
                var suffix = (dir + '/' + file.file).substr(blockPartL - 1);
