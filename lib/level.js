...
var Level = exports.Level = INHERIT(/** @lends Level.prototype */{
    ...
    __constructor: function(path) {
        ...
        this.cache = useCache;
        if (~exceptLevels.indexOf(path)) this.cache = !this.cache;
        ...
        if (this.cache) {
            return this.filesPromise = bemUtil.readFile(PATH.join(_this.dir, 'files.json'))
                .then(function(cacheContent) {
                    ...
                    LOGGER.fdebug('level %s read from cache', _this.dir);
                })
                .fail(function(err) {
    ...
    _scanFiles: function() {
        ...
        return bemUtil
            ...
            .fail(function(err) {
                LOGGER.fsilly('Failed to scan level %s: %s', _this.dir, err.message);
            })
            .then(function() {
                LOGGER.timeEndLevel('silly', 'scan ' + _this.dir);
