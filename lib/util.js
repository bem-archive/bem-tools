...
exports.readJsonJs = function(path) {
    return exports.readFile(path).then(function(source) {
        var context = VM.createContext(),
            dirname = PATH.dirname(path),
            promises = [],
            json;

        context.include = function(relpath) {
            var includePath = PATH.normalize(PATH.join(dirname, relpath)),
                promise;

            promise = exports.readJsonJs(includePath);
        ...
        json = VM.runInContext(source, context, path);
