...
exports.Server = INHERIT({
    ...
    createRequestHandler: function(root, runner) {
        ...
        return function(request, response) {
            ...
            LOGGER.time('[t] Build total for "%s"', relPath);
            _this._targetsInProcess++;
            return runner.findAndProcess(relPath)
                .fin(function() {
                    _this._targetsInProcess--;
                    ...
                    LOGGER.timeEnd('[t] Build total for "%s"', relPath);
                })
                .fail(function(err) {
                    if (typeof err === 'string') {
                        LOGGER.fverbose('*** node %s not found or failed to build: %s', relPath, err);
                        return;
                    }
                    return Q.reject(err);
                })
