...
function showInfo(opts) {
    return function() {
        LOGGER.info(UTIL.format("Project root is '%s'", opts.root));
        LOGGER.fverbose(UTIL.format('Options are %j', opts));
    }
}
...
function startServer(root, port, hostname) {
    ...
    return function(runner) {
        ...
        return server.listen(port, hostname).then(function() {
            LOGGER.info(UTIL.format('Server is listening on port %s', port));
            return server;
        });
    }
...
function requestHandler(root, runner) {
    ...
    return function(request, response) {
        ...
        LOGGER.verbose(UTIL.format('*** trying to access %s', fullPath));

        // try to find node in arch
        LOGGER.verbose(UTIL.format('*** searching for node "%s"', relPath));
        return runner.findNode(relPath)
            .fail(function(err) {
                if (typeof err === 'string') {
                    LOGGER.verbose(UTIL.format('*** node not found "%s"', relPath));
                    return;
                }
                return Q.reject(err);
            })
            .then(function(id) {
                ...
                // found, run build
                LOGGER.verbose(UTIL.format('*** node found, building "%s"', id));
